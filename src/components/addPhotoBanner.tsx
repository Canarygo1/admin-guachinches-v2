...
import {AddBanner, createBanner} from "../Data/obj/banner";

interface AddPhotoBannerDialogProps {
  open: boolean;
  onClose: () => void;
  orden: number;
}

const AddBannerPhotoDialog: React.FC<AddPhotoBannerDialogProps> = ({ open, onClose, orden }) => {
  const [file, setFile] = useState<string | ArrayBuffer | null>(null);
  const [loading, setLoading] = useState(false);
  const queryClient = useQueryClient();

  const addPhotoMutation = useMutation({
    mutationFn: (args: AddBanner) => createBanner(args),
    onSuccess: () => {
  ...
  const handleAddPhoto = () => {
    if (typeof file === 'string') {
      addPhotoMutation.mutate({image:file, orden});
    }
  };
...
export default AddBannerPhotoDialog;
